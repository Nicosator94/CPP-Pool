...
/*   Updated: 2024/04/15 16:19:01 by niromano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
DiamondTrap::DiamondTrap(const std::string &name) : ClapTrap(name + "_clap_name"), ScavTrap(name), FragTrap(name), _diamondName(name) {
	std::cout << "DiamondTrap : Constructor called for " << this->_diamondName << std::endl << std::endl;
...
DiamondTrap::DiamondTrap(const DiamondTrap &copy) : ClapTrap(copy), ScavTrap(copy), FragTrap(copy), _diamondName(copy._diamondName) {
	std::cout << "DiamondTrap : Copy constructor called by " << this->_diamondName << std::endl << std::endl;
...
DiamondTrap& DiamondTrap::operator=(const DiamondTrap &diamondtrap) {
	std::cout << "DiamondTrap : Assignement operator called for " << this->_diamondName << " with " << diamondtrap._diamondName << std::endl << std::endl;
